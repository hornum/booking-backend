...
async def test_admin_deactivate_user(user_repo, booking_repo, base_user_model_data):
    user = await user_repo.add(User(**base_user_model_data))
...
async def test_admin_activate_user(user_repo, booking_repo, base_user_model_data):
    user = await user_repo.add(User(**base_user_model_data, is_active=False))
...
async def test_deactivation_cancels_all_active_user_bookings(
    ...
    hold_booking = await booking_repo.add(
        Booking(
            **{
                **base_booking_model_data,
                "user_id": user.id,
    ...
    confirmed_booking = await booking_repo.add(
        Booking(
            **{
                **base_booking_model_data,
                "user_id": user.id,
    ...
    already_cancelled_booking = await booking_repo.add(
        Booking(
            **{
                **base_booking_model_data,
                "user_id": user.id,
    ...
    updated_hold = await booking_repo.get(hold_booking.id)
    updated_confirmed = await booking_repo.get(confirmed_booking.id)
    updated_cancelled = await booking_repo.get(already_cancelled_booking.id)
...
async def test_deactivation_does_not_cancel_other_user_bookings(
    ...
    target_user = await user_repo.add(User(**base_user_model_data))
    other_user = await user_repo.add(
        User(
            **{
                **base_user_model_data,
                "username": "other-user",
    ...
    target_booking = await booking_repo.add(
        Booking(
            **{
                **base_booking_model_data,
    ...
    other_user_booking = await booking_repo.add(
        Booking(
            **{
                **base_booking_model_data,
    ...
    updated_target_booking = await booking_repo.get(target_booking.id)
    updated_other_booking = await booking_repo.get(other_user_booking.id)
...
async def test_activation_does_not_change_bookings(
    ...
    booking = await booking_repo.add(Booking(**booking_data))
    await admin_change_user_status(
        user_repo=user_repo,
        booking_repo=booking_repo,
        new_status=True,
    ...
    updated_booking = await booking_repo.get(booking.id)
