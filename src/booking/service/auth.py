...
async def register_user(
    ...
    user = User(
        username=username, email=email, hashed_password=await hash_password(password)
    )
    user = await user_repo.add(user)
    if user.id is None:
        raise RuntimeError("Repository did not assign id after add")
...
async def login_user(
    user_repo: UserRepository, token_repo: TokenRepository, username: str, password: str
) -> LoginResult:
